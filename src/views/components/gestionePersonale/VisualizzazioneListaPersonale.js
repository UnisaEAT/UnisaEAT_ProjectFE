...
export default class VisualizzazioneListaPersonale extends React.Component {
  ...
  componentDidMount () {
    if (!localStorage.getItem('email')) { this.setState({ error: 400 }) } else if (localStorage.getItem('ruolo') != 'admin' && localStorage.getItem('ruolo') != 'personale adisu') { this.setState({ error: 401 }) } else {
            ...
            {this.state.utente.map((oggetto, i) => {
              return (
                ...
                >
                  ...
                  >
                    <Image
                      src='https://image.flaticon.com/icons/png/512/61/61403.png'
                      width='35'
                    />
                  </Button>
                </ListGroup.Item>
