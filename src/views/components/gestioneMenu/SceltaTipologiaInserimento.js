...
export default class SceltaTipologiaInserimento extends React.Component {
  ...
  componentDidMount () {
    if (!localStorage.getItem('email')) { this.setState({ error: 400 }) } else if (localStorage.getItem('ruolo') != 'operatore mensa') { this.setState({ error: 401 }) }
  }

  render () {
    if (this.state.error === 400) { return <h1 className='erroreGenericoDiAccesso'>Effettua il login per accedere a questa pagina</h1> }
    if (this.state.error === 401) { return <h1 className='erroreGenericoDiAccesso'>Accesso negato</h1> }
      ...
      <div id='root' className='rootContainer'>
        ...
        <div className='tipologiaContainer mt-4'>
          <Form>
            ...
            <Button
              className='buttonStyleTip mt-4' variant='outline-primary' type='submit'
