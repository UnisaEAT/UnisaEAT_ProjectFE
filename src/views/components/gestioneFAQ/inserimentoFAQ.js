...
export default class InserimentoFAQ extends React.Component {
  ...
  componentDidMount () {
    if (!localStorage.getItem('email')) { this.setState({ error: 400 }) } else if (localStorage.getItem('ruolo') != 'personale adisu') { this.setState({ error: 401 }) }
  }
  ...
  submitInserimentoForm (FAQ) {
    axios.post('http://localhost:8080/api/faq/insertFAQ', FAQ)
      .then(response => {
        ...
        } else if (response.data.message != true) { console.log(response.data.message) }
  ...
  render () {
    if (this.state.error === 400) { return <h1 className='erroreGenericoDiAccesso'>Effettua il login per accedere a questa pagina</h1> } else if (this.state.error === 401) { return <h1 className='erroreGenericoDiAccesso'>Accesso negato</h1> } else if (this.state.redirect) {
