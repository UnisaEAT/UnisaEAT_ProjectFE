        ...
        .then(response => {
          ...
          axios.post('http://localhost:8080/api/ordine/ordinaPasti', { ruolo: localStorage.getItem('ruolo') })
            .then(response2 => {
              ...
              else if (response.data.pranzo && response.data.cena === false) { this.handleError('Oggi hai già ordinato per pranzo') }
              ...
              else { this.handleError('Oggi hai già ordinato sia per pranzo che per cena') }
            })
            .catch((error) => {
              console.log(error)
            })
