...
export default class ListaOrdini extends React.Component {
  ...
  render () {
    if (this.state.error === 400) { return <h1 className='erroreGenericoDiAccesso'>Effettua il login per accedere a questa pagina</h1> } else if (this.state.error === 401) { return <h1 className='erroreGenericoDiAccesso'>Accesso negato</h1> } else if (this.state.ordini.length > 0) {
      return (
        <div className='lo-listaContainerInfo'>
          <table className='lo-table'>
            ...
            <tr className='lo-rowNoOp'><td /></tr>
            {
                    this.state.ordini.map((ordine, i) => {
                      let boolPranzo = 'Cena'
                      if (ordine.boolPranzo) { boolPranzo = 'Pranzo' }
